...
set "LOGFILE=%LOGDIR%\buildBulletCMake.log"
echo === buildBulletCMake started at %DATE% %TIME% > "%LOGFILE%"
...
echo === Building Bullet in dir %BULLETDIR% for arch %BULLETMACH% into bullet-build >> "%LOGFILE%"
pushd "%BUILDROOT%"

echo === [1/2] CMake configure laeuft ... (Ausgabe geht in Log)
cmake -G "%BULLETCMAKE_GENERATOR%" -A %BULLETMACH% ^
    ...
    -DCMAKE_BUILD_TYPE=Release %BULLETCMAKE_ARGS% .. >> "%LOGFILE%" 2>&1
if errorlevel 1 ( echo === [1/2] CMake configure FEHLGESCHLAGEN - siehe %LOGFILE% & popd & exit /b 1 )
echo === [1/2] CMake configure erfolgreich

echo === [2/2] CMake Build laeuft ... (kann mehrere Minuten dauern)
cmake --build . --config Release >> "%LOGFILE%" 2>&1
if errorlevel 1 ( echo === [2/2] CMake Build FEHLGESCHLAGEN - siehe %LOGFILE% & popd & exit /b 1 )
echo === [2/2] CMake Build erfolgreich
...
echo === buildBulletCMake completed at %DATE% %TIME% >> "%LOGFILE%"
